...
echo "$($PSQL "SELECT unnest(array[t1.name, t2.name]) as t3 FROM games INNER JOIN teams t1 ON games.winner_id = t1.team_id INNER JOIN teams t2 ON t2.team_id = games.opponent_id WHERE year=2014 and round='Eighth-Final' ORDER BY t3")"
